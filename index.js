...
http.createServer((req, res) => {
  if (req.url === '/qr') {
    if (!currentQR) {
      ...
      res.end('<html><head><meta charset="utf-8"><meta http-equiv="refresh" content="5"></head><body style="text-align:center;font-family:Arial;padding:50px;background:#f0f0f0"><h2>جاري تحميل QR Code...</h2><p>الصفحة تتحدث تلقائياً كل 5 ثواني</p></body></html>');
      return;
    }
    QRCode.toDataURL(currentQR, {width:300}, (err, url) => {
      res.writeHead(200, {'Content-Type':'text/html;charset=utf-8'});
      res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="60"><style>body{font-family:Arial;text-align:center;padding:30px;background:#f0f0f0}img{border:8px solid #25D366;border-radius:12px;margin:20px}h2{color:#075E54}</style></head><body><h2>امسح الكود بواتساب</h2><img src="${url}" width="300"/><p>واتساب ← الأجهزة المرتبطة ← ربط جهاز</p><p style="color:#999;font-size:13px">الصفحة تتحدث كل دقيقة</p></body></html>`);
    });
  } else {
    res.writeHead(200, {'Content-Type':'text/html;charset=utf-8'});
    res.end('<html><body style="text-align:center;font-family:Arial;padding:50px;background:#f0f0f0"><h2>مطعم O2 Bot ✅</h2><p>البوت شغال</p><a href="/qr" style="display:inline-block;margin-top:20px;background:#25D366;color:white;padding:14px 28px;border-radius:8px;text-decoration:none;font-size:18px">عرض QR Code</a></body></html>');
  }
...
const MENU_ITEMS = [
  ...
  {name:'شاورما إيطالي',cat:'شاورما',price:38,keys:['شاورما ايطالي','شاورما إيطالي','شاورما طلياني']},
  ...
  {name:'بيتزا خضار وذرة وزيتون',cat:'ايطالي',price:20,keys:['بيتزا خضار','خضار وذرة','بيتزا ذرة']},
  {name:'بيتزا ماما روزا',cat:'ايطالي',price:20,keys:['ماما روزا','بيتزا ماما','ماما']},
  {name:'نابولي',cat:'ايطالي',price:20,keys:['نابولي']},
  {name:'مارغريتا',cat:'ايطالي',price:20,keys:['مارغريتا','مرغريتا','مارجريتا']},
  ...
  {name:'فطيرة ذهبية',cat:'ساندويش',price:30,keys:['فطيرة','فطيره','فطيرة ذهبية']},
  ...
  {name:'سلطات وسط',cat:'سلطة',price:10,keys:['سلطة وسط','سلطات وسط','سلطة']},
  ...
  {name:'قهوة تركي سنجل',cat:'مشروبات',price:5,keys:['قهوة تركي','تركي سنجل','تركي']},
  ...
  {name:'تشيز كيك',cat:'حلويات',price:10,keys:['تشيز كيك','تشيزكيك']},
...
function isOrder(text) {
  return /بدي|عايز|اريد|أريد|اطلب|خذلي|حاطط|اضيف|زود/.test(text);
}
...
async function handleMessage(msg) {
  ...
  if (!session.state) {
    ...
    if (/^[1-6]$/.test(text)) {
      ...
      return getMenuText(cats[parseInt(text)-1]);
    }
  ...
  if (session.state === 'pending_item') {
    ...
    if (/نعم|آه|اه|اوك|ok|يلا|تمام|اضيف|طلب|بدي/.test(text)) {
      session.state = 'ordering';
      session.cart.push({...item, qty:1});
      session.pendingItem = null;
      return `تمام أضفته! 🛒\nشو كمان بدك؟ 😊`;
    }
    if (/لا|لأ|بس|سعر/.test(text)) {
      session.state = null;
      session.pendingItem = null;
  ...
  if (session.state === 'ordering') {
    ...
    if (qtyMatch) {
      ...
      return `مش لاقي "${query}" 🤔 تأكد من الاسم أو أرسل *منيو*`;
    }
  ...
  if (session.state === 'delivery_type') {
    if (/^1$|توصيل|ديليفري|بيتي|دار|منزل/.test(text)) {
      session.deliveryType = 'توصيل';
      session.state = 'address';
      return `تمام! 🚚 أرسل عنوانك التفصيلي\n(المنطقة + الشارع)`;
    }
    if (/^2$|استلام|مطعم|بجي|برجع|بمشي/.test(text)) {
  ...
  if (session.state === 'address') {
    ...
    session.state = 'name';
    return `تمام! رسوم التوصيل لـ${raw}: *${session.deliveryFee} ₪* 🚚\n\nشو اسمك الكريم؟ 😊`;
  }
  ...
  if (session.state === 'phone') {
    if (!/^[\d\s\-\+]{7,15}$/.test(raw))
      return `الرقم مش واضح 😅 أرسله مرة ثانية 📞`;
    session.phone = raw;
    session.state = 'confirm';
    const total = cartTotal(session.cart);
    const grand = total + session.deliveryFee;
    return `تمام! هاد ملخص طلبك 📋\n─────────────\n${cartText(session.cart)}\n─────────────\nالطلب: ${total} ₪\n${session.deliveryType==='توصيل'?`التوصيل (${session.address}): ${session.deliveryFee} ₪`:'استلام من المطعم 🏪'}\n*المجموع: ${grand} ₪*\n─────────────\n👤 ${session.name}  📞 ${session.phone}\n\nكل شي صح؟\n✅ أرسل *تأكيد*\n✏️ أرسل *تعديل*\n❌ أرسل *إلغاء*`;
  }
  ...
  if (session.state === 'confirm') {
    ...
    return `أرسل *تأكيد* ✅ أو *تعديل* ✏️ أو *إلغاء* ❌`;
  }
...
console.log('Chrome path:', chromePath || 'سيستخدم puppeteer المدمج');
...
client.on('qr', qr => {
  currentQR = qr;
  console.log('QR Code جاهز — افتح رابط /qr لمسحه');
...
client.on('auth_failure', () => {
  console.log('فشل التحقق — جاري إعادة المحاولة');
});
